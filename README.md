...
- ログインID: test1, test2, ... (20人分)
- パスワード: password（全員共通）

### モデレーター
- URL: http://localhost:49152/moderators/sign_in
- ログインID: {tenant_slug}_1, {tenant_slug}_2, {tenant_slug}_3（各テナントに3人）
- パスワード: password（全員共通）
- 例: テナントslugが `tenant-1-abc123` の場合、`tenant-1-abc123_1`
...
| 用語                                 | 説明                                             |
| ---------------------------------- | ---------------------------------------------- |
| **テナント**（Tenant）                   | 掲示板の単位。1つのテナントに複数のお題、メンバー、モデレーターが所属する。 |
| **テナント招待**（Tenant Invitation）      | テナントへの招待。メンバーがメールアドレスで他ユーザーを招待できる。 |

#### ユーザー関連

| 用語                          | 説明                                             |
| --------------------------- | ---------------------------------------------- |
| **会員ユーザー**（Member User）     | 会員登録をしたユーザー。テナントに参加することでお題を投稿したり、コメントを投稿できる。|
| **非会員ユーザー**（Guest User）     | 会員登録をしていないユーザー。掲示板の閲覧はできるが、投稿やコメントはできない。|
| **モデレーター**（Moderator）       | テナントの運営を行うユーザー。テナントに所属し、通報の審査や通報理由の管理を行う。 |
| **表示名**（Display Name）       | テナント内でのユーザーの表示名。テナントごとに異なる表示名を設定できる。 |

#### コンテンツ関連

| 用語                                 | 説明                                   |
| ---------------------------------- | ------------------------------------ |
| **お題**（Topic）                      | 掲示板に集まった人で話し合うためのテーマ。テナントのメンバーであれば誰でも立てることができる。|
...
| 用語                           | 説明                                 |
| ---------------------------- | ---------------------------------- |
| **通報**（Report）               | コメントの非表示またはユーザーの利用停止を求める行為。       |
| **通報対象タイプ**（Reportable Type）     | 通報対象が「コメント非表示」か「ユーザー停止」かを示す区分。 |
| **通報理由**（Ban Reason）         | 通報理由の種別。テナントごとに管理され、デフォルトでは「スパム」「嫌がらせ」「わいせつ」「その他」がある。 |
| **通報理由テキスト**（Reason Text）    | 通報の具体的な理由                    |
| **通報者**（Reporter）            | 通報を行ったユーザー。                        |
| **通報対象ユーザー**（Reportable User）    | ユーザ停止の対象者ユーザ                |
| **通報対象コメント**（Reportable Comment） | コメント非表示の対象のコメント                 |
| **審査**（Decision）             | 通報が適切かどうかを判定する行為。モデレーターが行う。        |
| **審査タイプ**（Decision Type）     | 審査の結果。「却下」「コメント非表示」「ユーザー利用停止」がある。  |
| **審査メモ**（Note）               | 審査する際のメモ                   |
| **審査者**（Decided By）          | 審査を行ったモデレーター。                      |
| **利用停止**（Suspension）         | ユーザーの利用を一時的に停止すること。停止期間中はテナント内での投稿ができない。 |
